...
# 遊戲模式
1. 透過 socket，兩位玩家可以互相對戰
   1. socket 的連線主要在於同步傳遞猜測資料
2. 兩個玩家都準備好了，遊戲才會開始
3. 玩家需要自行設定自己的數字，必須為四位數，1-9 不能重複
   1. 實作數字鍵盤，讓玩家可以快速輸入數字
   2. 實作隨機生成數字，讓玩家可以快速生成數字
4. 玩家可以猜測對手的數字，猜測數字後，會回傳猜測的結果，例如 `1A2B`，表示有1個數字猜對，且有2個數字位置猜錯
5. 猜測次數不限制，但是每猜一次
   1. 預設遊戲會存到記憶體中，如果想要永久保存，可以將遊戲資料存到 redis 中
   2. 遊戲的紀錄會存到 redis 中，redis 會在1小時後過期
...
4. 實作遊戲重置功能，讓玩家可以在遊戲結束後重新開始遊戲
5. 增加遊戲超時機制，若玩家在一定時間內未進行操作，則自動結束遊戲
